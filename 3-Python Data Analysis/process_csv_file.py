...
def filter_by_year(statistics, year, yearid):
    ...
    for stats in statistics:
        if stats[yearid] == str(year):
            search_year.append(stats)
    return search_year
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    top_player = []
    for stats in statistics:
        playerid = (stats[info['playerid']])
        player = [playerid, formula(info, stats)]
        top_player.append(player)

        top_player_sorted = sorted(top_player, key=lambda stat: stat[1], reverse=True)

    top_numplayers = top_player_sorted[:numplayers]

    return [tuple(list_list) for list_list in top_numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    read_file = read_csv_as_nested_dict(info["masterfile"], info["playerid"],
                                        info['separator'], info['quote'])

    for player in top_ids_and_stats:
        firstname = read_file[player[0]][info["firstname"]]
        lastname = read_file[player[0]][info["lastname"]]

        lookup_player_list.append("{0:0.3f} --- {1} {2}".format(player[1], firstname, lastname))

    return lookup_player_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    battingfile_info = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    stats_year = filter_by_year(battingfile_info, year, info['yearid'])

    top_players = top_player_ids(info, stats_year, formula, numplayers)

    top_players_record = lookup_player_names(info, top_players)

    return top_players_record
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for stats in statistics:
        if stats[playerid] not in aggregated_player:
            aggregated_player[stats[playerid]] = {playerid: stats[playerid]}

            for field in fields:
                aggregated_player[stats[playerid]][field] = 0

        for field in fields:
            aggregated_player[stats[playerid]][field] += int(stats[field])

    return aggregated_player
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    top_stats = aggregate_by_player_id(statistics, info['playerid'], info['battingfields'])

    stats_list = []
    for value in top_stats.values():
        stats_list.append(value)

    id_and_stats = top_player_ids(info, stats_list, formula, numplayers)

    return lookup_player_names(info, id_and_stats)
